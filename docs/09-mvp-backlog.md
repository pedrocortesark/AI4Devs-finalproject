...
|----|--------|----|-----------  |----------------|-----|
| `T-0500-INFRA` | **Setup React Three Fiber Stack** | 2 | Instalar `@react-three/fiber@^8.15`, `@react-three/drei@^9.92`, `three@^0.160`, `zustand@^4.4`. Configurar Vite para GLB assets, TypeScript types. **Incluye:** gltf-pipeline CLI para Draco compression (npm install -g). POC validó stack con 60 FPS, 41 MB memory. | [T-0500-INFRA-TechnicalSpec.md](US-005/T-0500-INFRA-TechnicalSpec.md) | Dependencies installed, GLB imports work, Canvas3DTest renders, gltf-pipeline compresses 778KB → <400KB |
| `T-0501-BACK` | **List Parts API - No Pagination** | 3 | Endpoint `GET /api/parts` retorna ALL parts (no paginación, canvas necesita todo). Añadir `low_poly_url`, `bbox` en response. Filters: `status`, `tipologia`, `workshop_id` (SQL WHERE). RLS: workshop users ven solo assigned+unassigned. Response optimizada <200KB, query <500ms con index `idx_blocks_canvas_query`. | [T-0501-BACK-TechnicalSpec.md](US-005/T-0501-BACK-TechnicalSpec.md) | ALL parts returned, low_poly_url included, filters work, RLS applies, response <200KB <500ms, uses index |
| `T-0502-AGENT` | **Generate Low-Poly GLB from .3dm** | 5 | Tarea Celery `generate_low_poly_glb(block_id)`. Leer .3dm con rhino3dm → Decimación 90% (39,360 tris → 1000 tris target) → Exportar GLB con gltf-pipeline Draco level 10 → S3 `processed-geometry/low-poly/`. **Incluye:** Fix Face tuple iteration (`len(f)==4` para quads), InstanceObjects support (export_instances_gltf.py pattern). POC validó 778KB sin Draco → 300-400KB con compresión. | [T-0502-AGENT-TechnicalSpec.md](US-005/T-0502-AGENT-TechnicalSpec.md) | GLB <500KB generated, decimation functional, Face tuple fix applied, low_poly_url populated in DB after processing |
| `T-0503-DB` | **Add low_poly_url Column & Indexes** | 1 | Migración: `ALTER TABLE blocks ADD COLUMN low_poly_url TEXT NULL, ADD COLUMN bbox JSONB NULL`. Indices: `idx_blocks_canvas_query ON (status, tipologia, workshop_id) WHERE is_archived=false`, `idx_blocks_low_poly_processing ON (status) WHERE low_poly_url IS NULL`. | [T-0503-DB-TechnicalSpec.md](US-005/T-0503-DB-TechnicalSpec.md) | Columns exist, indexes created, query uses canvas index (EXPLAIN ANALYZE verified), migration <30s |
| `T-0504-FRONT` | **Dashboard 3D Canvas Layout** | 3 | Componente `Dashboard3D.tsx`: Grid layout 80% Canvas + 20% Sidebar. `<Canvas shadows dpr={[1,2]}>` con `OrbitControls`, `Grid [100x100]`, `Stats` panel. Lighting setup: ambientLight + directionalLight. Responsive: <768px collapsa sidebar a bottom panel. EmptyState cuando `parts.length === 0`. LoadingOverlay durante fetch. | [T-0504-FRONT-TechnicalSpec.md](US-005/T-0504-FRONT-TechnicalSpec.md) | Canvas renders grid, OrbitControls work, Sidebar visible, EmptyState shows when no parts, mobile responsive, Stats panel dev-only |
| `T-0505-FRONT` | **3D Parts Scene - Low-Poly Meshes** | 5 | Componente `PartsScene.tsx`: Renderiza N piezas con `useGLTF(part.low_poly_url)`. **Incluye:** Z-up rotation fix (`scene.rotation.x = -Math.PI / 2` from POC), auto-camera centering (bbox calculation, camera at maxDim * 1.5), Suspense boundary con Loader. Grid automático 10x10 spacing. Color por status. POC validó 60 FPS con 1197 meshes. | [T-0505-FRONT-TechnicalSpec.md](US-005/T-0505-FRONT-TechnicalSpec.md) | 50+ geometries render, Z-up orientation correct, camera auto-centers, colors by status, FPS >30 maintained, Suspense fallback works |
| `T-0506-FRONT` | **Filters Sidebar & Zustand Store** | 3 | Zustand store `usePartsStore`: state `{ parts, filters, selectedId }`, actions `setFilters`, `selectPart`, computed `filteredParts()`. Sidebar: CheckboxGroup (Tipología, Status), Select (Workshop). URL sync: `?status=validated&tipologia=capitel`. Canvas fade-out non-matches (opacity 0.2). Counter "Mostrando X de Y". | [T-0506-FRONT-TechnicalSpec.md](US-005/T-0506-FRONT-TechnicalSpec.md) | Filters update store, canvas re-renders filtered, URL params synced, page reload maintains filters, fade-out smooth <300ms, counter accurate |

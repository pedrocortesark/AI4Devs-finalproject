...
### Corrección 2026-05-18 (c) — El bug era el conteo de objetos, NO el fixture (supersede (b))

- **Hallazgo (el usuario tenía razón):** La "dependencia abierta" de (b) era **incorrecta**. Los 102 Breps en capas `T_Jnt/ref/T_Tde/T_Brg/T_Pxn` **no son geometría suelta**: son la **geometría interna de las 6 InstanceDefinitions**. rhino3dm expone esa geometría de definición dentro de `model.Objects` (object table del `.3dm`), pero NO son objetos colocados a nivel documento.
- **Evidencia empírica:** `sum(len(d.GetObjectIds()) for d in InstanceDefinitions) == 102`; clasificando `model.Objects` por `Attributes.IsInstanceDefinitionObject`: `(Brep, True)=102` (internos de definición) y `(InstanceReference, False)=6` (top-level reales). Los objetos a nivel documento son **exactamente 6 InstanceReference**.
- **Causa raíz (mi bug):** `validate_geometry` iteraba **todos** `model.Objects` en vez de filtrar la geometría interna de las definiciones. El reexport del usuario era correcto desde el principio.
- **Fix:** `validate_geometry` ahora salta los objetos con `Attributes.IsInstanceDefinitionObject == True` y valida solo los objetos top-level: deben ser exclusivamente `InstanceReference` (≥1), cada uno válido + bbox + volumen. `test-model.3dm` → `total_objects=108`, `top_level=6`, `instances=6`, **0 errores → VALIDA**.
- **Tests:** Mocks ajustados con `obj.Attributes.IsInstanceDefinitionObject = False` (autouse `test_stategraph.py`, `mock_rhino_model_valid`/`invalid_geometry` en `test_stategraph_validators.py`). Suite unit del agente: **72 passed, 0 failed** (determinista). Se ANULA la dependencia de reexport de (b): los 3 happy-path validan con el `test-model.3dm` real tal cual.

### Cierre 2026-05-18 (d) — `make test-agent` pre-roto + 2 landmines de import backend en el worker real

- **`make test-agent` está roto de antes (no es regresión):** Probado con baseline en árbol limpio (todos los cambios en stash): mismos 5 errores de colección e idéntico `Error 2`. Causa: el target ejecuta pytest sobre `tests/unit/` (con `--ignore` incompletos) e incluye tests de backend que el contenedor `agent-worker` no puede importar (`test_preview`→`fastapi`, `test_prometheus_service`→`prometheus_client`, `test_geometry_validator`/`test_metrics_service`/`test_rate_limiter`→cadena `services/__init__`→`schemas`). pytest aborta toda la sesión → 0 tests ejecutados. Es deuda de infra del Makefile, fuera del alcance de esta tarea.
- **2 landmines pre-existentes de import, expuestos por el wiring en el worker real:** Los nodos LangGraph se escribieron asumiendo el namespace del contenedor backend y nunca corrieron en `agent-worker` hasta el wiring. (1) `geometry_validator.py`: `from schemas import ValidationErrorItem` con try/except de ramas idénticas (sin fallback) → `ModuleNotFoundError` en agent-worker. (2) `node_extract_geometry`: `from constants import STORAGE_BUCKET_RAW_UPLOADS` → en agent-worker `constants`=constantes del agente (no tiene el símbolo). **Fix (mismo patrón cascade):** `try: <import bare> except (Import/ModuleNotFound): from src.backend.<mod> import …`. Resto de imports de runtime de nodos verificados OK en agent-worker (infra.redis_client, llm_client, circuit_breaker, classification_helpers, jinja2).
- **Verificación end-to-end real (no solo unit):** `docker compose run --rm agent-worker python -m pytest tests/integration/test_validate_file_task.py -v` (esquiva la colección rota), **contra Supabase real**: **7 passed, 4 skipped, 0 failed**. `test-model.3dm` real (6 block instances) **valida end-to-end en el contenedor de producción**; caso negativo (sin instancias) rechazado; manejo de errores OK. Los 4 skipped son skips intencionados pre-existentes.
- **Conclusión:** Con nomenclatura fuera + geometría por block-instances (top-level) + estos 2 fixes de import, el pipeline LangGraph cableado funciona de verdad en el worker real con el modelo real de la Sagrada Família. Misma familia que el hardening previo de `llm_client`/`src.backend`.

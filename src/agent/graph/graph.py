...
The graph is designed to stop early when validation fails:
  ...
  1. ExtractGeometry (ingestion gatekeeper)
     → If file missing / parse failed → go to MarkRejected
     → Otherwise → continue to ValidateGeometry

  2. ValidateGeometry (quality gatekeeper)
     → If geometry_valid == False → skip LLM, go to MarkRejected
     → If geometry_valid == True → continue to ClassifyTipologia

NOTE: ISO-19650 nomenclature validation was removed. Real Sagrada Família
.3dm layer names (e.g. 'Peces', 'PER-Pere', 'T_Pxn') never follow ISO-19650,
so that gatekeeper rejected legitimate files. See memory-bank/decisions.md.

This architecture saves:
  - API costs (no LLM call if geometry fails)
  - Processing time (early rejection)
  - Storage I/O
...
START
  → ExtractGeometry
    → [file missing/parse fail] → MarkRejected → END
...
IMPLEMENTATION:
==================================
  - 7 nodes (nomenclature node removed)
...
def should_continue_after_extract_geometry(state: ValidationState) -> Literal["ValidateGeometry", "MarkRejected"]:
    """
    Conditional edge after ExtractGeometry node.

    Returns:
        "ValidateGeometry" if file_exists_in_storage == True (continue happy path)
        "MarkRejected" if file download/parse failed (fail-fast)

    This is the only ingestion gatekeeper — if the file doesn't exist or fails
    to parse we reject immediately. ISO-19650 nomenclature validation was
    removed: real Sagrada Família .3dm layer names never follow ISO-19650, so
    that check rejected legitimate files (see memory-bank/decisions.md).
    """
    geometry_metadata = state.get("geometry_metadata", {})
    file_exists = geometry_metadata.get("file_exists_in_storage", False)
    next_node = "ValidateGeometry" if file_exists else "MarkRejected"
...
def create_validation_graph() -> StateGraph:
    ...
    """
    logger.info("graph.building", nodes=7, conditional_edges=2)
    ...
    # → If file_exists_in_storage == True → ValidateGeometry
    # → If file download/parse failed → MarkRejected (fail-fast)
    # (ISO-19650 nomenclature gatekeeper removed — see decisions.md)
    ...
    logger.info(
        "graph.compiled",
        nodes=7,  # nomenclature node removed
        edges=7,  # 2 conditional + 5 normal
